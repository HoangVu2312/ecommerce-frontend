// This component is to change product data (Only for Admin)
...
import "./NewProduct.css";  // same style as add new product page

function EditProductPage() {

    const { id } = useParams(); // productId from Dashboardproduct

    // set local initial state as the old product data
    ...
    useEffect(() => {
        // the destructed props from data.product is IDENTICAL as states => but there're contrain in this function => OKAY
        axios
            .get(`/products/${id}`)  

            .then(({ data }) => { // data = {product, similar} => same router as ProductPage
                const{name, description, category, pictures, price} = data.product; // get that specific product, not sililar
                setName(name);
    ...
    return (
        <Container>
            <Row>
                ...
                <Col md={6} className="update-product__image--container"></Col>
            </Row>
        </Container>
    );
}
